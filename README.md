...
| **Idle** | Time since the PR author last pushed, commented, or replied |
| **Sign-off** | Present `approved-*` and `lgtm-*` labels (green); `-` if none |
| **CI** | Failing check names, one per line (red), or green `all pass` |
| **Pending** | Personal action links, one per line (see below) |

### Pending links

Grouped signals in the **Pending** column:

| Link | Links to |
|------|----------|
| `started(N)` | Unresolved review thread you started |
| `joined(N)` | Unresolved thread you joined |
| `mention(N)` | @mention without your later reply |
| `new-commits` | Latest commit on the PR |
| `reviews(1): user` | A review submitted after yours (one link per reviewer) |

The `pr` subcommand shows the same detail in a panel, including sign-off labels, CI status, and full URL lists.
...
```json
{
  ...
  "last_author_activity_at": "2026-06-17T09:00:00+00:00",
  "labels": ["approved-qe", "bug"],
